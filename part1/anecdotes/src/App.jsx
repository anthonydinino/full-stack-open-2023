...
const App = () => {
  ...
  const [AofTheDay, setAofTheDay] = useState(0);

  const assignVote = (selected) => {
    let newVotes = [...votes];
    newVotes[selected] += 1;
    setVotes(newVotes);
    setAofTheDay(newVotes.indexOf(Math.max.apply(0, newVotes)));
  };
  ...
  return (
    <>
      ...
      <Anecdote value={anecdotes[AofTheDay]} votes={votes[AofTheDay]} />
    </>
  );
};
